...
options:
    ...
    --send-mail           if given, program will try to send out new credentials by mail using pre-configured command template from the settings.ini file\
    --send-bot            if given, program will try to send out new credentials via a Telegram bot using pre-configured command template from the settings.ini file.
